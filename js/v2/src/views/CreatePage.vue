    ...
    <ion-page>
        <div style="height:100%; display: flex; align-items: center; justify-content: center;" class="gradient">
            <ion-card color="dark" style="width: 100%; max-width: 400px;">
                ...
                <ion-card-content>
                    <ion-input label="First Name" v-model="data.first_name" label-placement="stacked"
                        placeholder="First Name" />
                    <ion-input label="Last Name" v-model="data.last_name" label-placement="stacked"
                        placeholder="Last Name" />
                    <ion-input label="UMA Address" v-model="data.uma_address" label-placement="stacked"
                        placeholder="UMA Address">
                        <ion-button id="info-trigger" fill="clear" slot="end" aria-label="Show/hide">
                            <ion-icon slot="icon-only" :icon="informationCircle" aria-hidden="true"></ion-icon>
                        </ion-button>
                        <ion-popover trigger="info-trigger" trigger-action="click" side="left" alignment="start">
                            <ion-content color="dark" class="ion-padding">
                                ...
                                <br />
                                Universal Money Addresses (UMA) allow you to connect your wallet and set a budget.
                                <br /><br />
                                Your favorite creators are paid in realtime, directly from your wallet as you listen.
                                <br /><br />
                                <ion-button expand="block" color="light" size="small" href="https://uma.me"
                                    target="_blank">
                                    Don't have an UMA?
                                </ion-button>
                            </ion-content>
                    ...
                    <template v-if="data.refresh_token">
                        <ion-text color="light">
                            <h3>Your UMA is connected!</h3>
                        </ion-text>
                        <br />
                        <ion-button color="light" expand="block" @click="createProfile">Finish</ion-button>
                    </template>
                    <template v-else>
                        <br />
                        <ion-button color="light" expand="block" @click="connectUMA">Connect</ion-button>
                    </template>
                </ion-card-content>
...
import { getAuthorizationUrl, getNWCConnection } from '@/uma';
import { IonPage, IonInput, IonButton, IonCard, IonCardHeader, IonCardTitle, IonCardContent } from '@ionic/vue';
...
const connectUMA = async () => {
    const authConfig = await getAuthorizationUrl(data.value.uma_address);
    const platform = Capacitor.getPlatform();
    if (platform === 'web') {
        // No in-app browser! Just set it and go. In the future, we might do the
        // redirect flow, but right now, web is just for local development.
        data.value.refresh_token = 'web';
        return;
    }

    InAppBrowser.addListener('urlChangeEvent', (state: UrlEvent) => {
        console.log(state);
        if (state.url.includes("localhost") && state.url.includes('code=')) {
            const code = new URL(state.url).searchParams.get('code') as string;
            getNWCConnection(code, authConfig.authorization_endpoint, authConfig.token_endpoint, authConfig.csrfState, authConfig.codeVerifier)
                .then((result) => {
                    console.log(JSON.stringify(result));
                    data.value.refresh_token = result.refresh_token!;
                    InAppBrowser.removeAllListeners();
                    InAppBrowser.close();
                });
        }
    });
    await InAppBrowser.open({ url: authConfig.authUrl.href });
}
...
const createProfile = async () => {
    const res = await updateUserProfile({
        body: {
            ...
            picture_url: '',
            nwc_string: '',
            nwc_refresh_token: data.value.refresh_token,
            nwc_expires_at: null,
